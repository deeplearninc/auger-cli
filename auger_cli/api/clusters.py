...
from auger_cli.formatter import print_record, print_line
...
def list_clusters(client, organization_id=None):
    params = {}
...
def read_cluster(client, cluster_id, attributes=None):
...
def cluster_is_running(client, cluster):
    return cluster.get('status') == 'running'

def create_cluster(client, organization_id, project_id,
...
def delete_cluster(client, cluster_id, wait=True):
